...
df$bin.Temp <- df$Temp<60
df$bin.Pressure <- df$Pressure<200
...
ggplot(df, aes(factor(O.ring), Pressure)) +  
  geom_boxplot(aes(fill = factor(O.ring))) + 
  geom_jitter() +
  ggtitle("Temp by Number of O ring failures") + 
  theme(plot.title = element_text(lineheight=1, face="bold")) 
...
```{r}
model2 <- glm(O.ring/Number ~ Temp + Pressure + Temp:Pressure, data = df, family = binomial, weights = Number)
summary(model2)
```
