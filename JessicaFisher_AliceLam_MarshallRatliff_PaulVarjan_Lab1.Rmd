...
# was plot(Pressure ~ Temp, data = df)
...
# was opres.plt <- plot(pressure_noise, df$O.ring)
...
```{r}
model1.binom <- glm(O.ring/Number ~ Temp + Pressure, data = df, family = binomial, 
                    weights=Number)
...
# Testing other Models before moving on
## Temp and Pressure with an Interaction term - an interaction term does not help improve the model.

```{r}
model2 <- glm(O.ring/Number ~ Temp + Pressure + Temp:Pressure, data = df, 
